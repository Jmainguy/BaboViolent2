...
void Client::recvPacket(char * buffer, int typeID)
{
	...
	case NET_SVCL_PROJECTILE_COORD_FRAME:
		...
		//	console->add(CString("Client recved projectile Coord Frame %i", (int)projectileCoordFrame.frameID));
			// Est-ce que notre player poc�de des projectiles au moins? Sinon il n'est peut etre pas cr�� encore (bug)
		...
		//		{
					// On doit searcher pour le bon, ils ont peut etre �t� d�call�
	...
	case NET_SVCL_PLAYER_SHOOT:
		{
			...
			if (game->players[playerShoot.playerID])
			{
				...
				// Si on touche un joueur, on spawn du SANG :D:D:D si c'est ff � on ou off pis que c un ennemy :(
	...
	case NET_SVCL_DELETE_PROJECTILE:
		{
			// �a �a ne devrait pas fucker, car on envoit dans l'ordre tout le temps (viva el TCP)
	...
	case NET_SVCL_GAME_STATE:
		{
			...
			{
				// Ouch, on fout toute � 0 (score, etc)
			}

			// On switch sur �a et on emet le son appropri�e
