...
#endif
{
	...
	if (scene->server || isEditor)
		fptr = &fileObj;
	// Joining client: use on-disk map when present (skip-download path); else streamed mapBuffer.
	else if (fileObj.isValid())
		fptr = &fileObj;
	else
	...
	if (!file.isValid())
	{
		...
		if (editor)
		{
			...
			// On cr� tout suite les contours de la map
			for (j=0;j<size[1];++j)
			{
	...
	//--- on check si on a pas un model 3D de cette map l�
	...
	{
		// On cr� l'espace pour la texture
...
#ifndef DEDICATED_SERVER
//
// Pour g�n�rer la texture de la minimap
//
void Map::regenTex()
{
    int i, j;
	...
	if (thisPlayer)
	{
		if (thisPlayer->status == PLAYER_STATUS_ALIVE)
		{
			if (thisPlayer->weapon)
			{
				...
				{
					//--- D�pendament de la distance entre la mouse et le player
...
bool Map::rayTest(CVector3f & p1, CVector3f & p2, CVector3f & normal)
{
	...
	// On pogne notre cell de d�part
	int i = (int)p1[0];
	int j = (int)p1[1];

	// On check que notre tuile n'est pas d�j� occup�
	if (i >= 0 &&
		i < size[0] &&
		j >= 0 &&
		j < size[1])
	...
	// On d�fini dans quel sens on va voyager (4 sens)
	int sens;
	...
	// On while tant qu'on ne l'a pas trouv�
	float percent;
	while (true)
	{
		// On check qu'on n'a pas d�pass�
		if (i < 0 ||
			i >= size[0] ||
			j < 0 ||
			j >= size[1] ||
		...
		case TEST_DIR_X:
			...
			// On incr�mente � la prochaine tuile
			i++;
...
void GetMapList(std::vector< CString > & maps)
{
	...
	std::snprintf(dirPath, sizeof(dirPath), "%s/main/maps", appPath);
	...
	while ((ent = readdir(dir)) != NULL)
	{
		...
		if (len <= 4)
			continue;
		if (strcasecmp(name + len - 4, ".bvm") != 0)
			continue;
