...
// Nos douilles, �a on g�re pas �a sur le net
struct Douille
{
	CVector3f position;
	CVector3f vel;
	...
	Douille(CVector3f pPosition, CVector3f pDirection, CVector3f right, int in_type=DOUILLE_TYPE_DOUILLE)
	{
		...
		{
			delay = 2; // �a dure 2sec �a, en masse
...
struct Projectile
{
	...
	CoordFrame netCF0; // L'avant dernier keyframe re�u du net
	CoordFrame netCF1; // Le dernier keyframe re�u du net
...
#ifndef DEDICATED_SERVER
	// La rocket tourne sur elle m�me
	float rotation;
	float rotateVel;
...
// Pour tenir nos explosion, �a c'est client OnLy
/*
struct Explosion
{
	CVector3f position;
...
public:
	...
	long mapSeed;
#ifndef DEDICATED_SERVER
	// Notre liste de trail � afficher
	std::vector<Trail*> trails;
#endif
	// notre liste de projectile (tr�s important de toujours les garder dans l'ordre
	...
	// Nos marques sur le plancher (�a c chouette)
...
public:
   ...
   // Create map
	...
	int createNewPlayerSV(int babonetID); // �a c'est c�t� server
#ifndef DEDICATED_SERVER
	void createNewPlayerCL(int playerID, long babonetID); // �a c'est c�t� client
	...
	// Pour afficher la minimap (ouff, je m�lange pomal les affaires, tk)
	void renderMiniMap();
