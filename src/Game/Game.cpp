...
//	uniqueProjectileID = 0;
   ...
   subGameType = gameVar.sv_subGameType;
	...
	// �a �a va �tre utilise quand on va �quilibrer les teams
	bluePing = 0;
	redPing = 0;
	ffaPing = 0;
	spectatorPing = 0;
...
void Game::resetRound()
{
	...
	// On respawn tout les players (le server va d�cider de tout �a)
	...
	if (thisPlayer && roundState == GAME_PLAYING)
	{
		...
		if (thisPlayer->status == PLAYER_STATUS_ALIVE) 
		{
				...
				(x < 0 || x > map->size[0] || y < 0 || y > map->size[1])))
			{
				...
				{
					// Ici on le call juste une fois, isshh sinon �a sera pas trop bon...
					// On request to spawn
...
void Game::shoot(const CVector3f & position, const CVector3f & direction, float imp, float damage, Player * from, int projectileType)
{
	if (map)
	{
		CVector3f p2 = direction * 128; // �a c'est le range, 128 c'est assez, grosseur max de map (c fucking big �a)
		...
		//	CVector3f dir = from->currentCF.mousePosOnMap - position; // Pas une bonne id�e �a, trop facile
		//	normalize(dir);
...
void Game::shootSV(int playerID, int nuzzleID, float imp, CVector3f p1, CVector3f p2)
{
	...
	// On test s'il y a une collision, sinon, fuck it on envoit pas �a
	if (map->rayTest(p1, p2, normal))
	{
		isCollision = true;
	}
...
void Game::createNewPlayerCL(int playerID, long babonetID)
{
	...
	{
		// On l'efface au cas quil existe d�j� (tr�s pas bon �a)
