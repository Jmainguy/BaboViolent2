...
cClient::cClient(sockaddr_in *ip, int fileDescriptor,unsigned long netID)
{
	UDPfd				=	0;
	UDPport				=	0;
	...
	FD_SET( FileDescriptor , &masterS );

	char *IP			=	inet_ntoa(ipAdress.sin_addr);
	unsigned short port		=	ntohs(ipAdress.sin_port);
...
cClient::cClient(const char *HostIP, unsigned short port,unsigned long netID)
{
...
int	cClient::ReceivePacketsFromServer()
{
    ...
    if (FD_ISSET(FileDescriptor, &read_fds))
	{
		...
		if((nbytes = recv(FileDescriptor,buf,3072,0)) <= 0)
		{
			//un probleme est survenu
			if(nbytes == 0)
			{
	...
	while(nread<nbytes)
	{
		...
		if(WaitingForKey)
		{
			...
			else
			{
				...
				// Packet id is 4 bytes in the key; do not copy sizeof(unsigned long) into pid[5] (stack corruption on LP64).
...
bool cClient::GetPendingID(char *pid)
{
	...
	snprintf(plain, sizeof(plain), "%lu", (unsigned long)PendingID);
...
int cClient::Send(UINT4 &nbByte)
{
	...
	if(PacketsToSend)
	{
		...
		//on parse notre key (4-byte RND1 tag; sizeof(sizeof(char)*4) is size_t and wrong on LP64)
...
int cClient::SendUDP(int UDPFD,unsigned long &nbByte)
{
