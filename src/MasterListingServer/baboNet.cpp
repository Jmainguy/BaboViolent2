...
//quelques proprietes globale
	...
	unsigned long		LastClientID=0;		// keep last emitted client ID
...
cClient *getClientByID(unsigned long clientID)
{
...
int	bb_serverSetClientRate(int nbBytes,unsigned long baboNetID)
{
	if(!Server) return 1;
...
unsigned long bb_serverGetBytesSent()
{
	if(Server)
	{
...
unsigned long bb_serverGetBytesReceived()
{
	if(Server)
	{
...
long bb_serverUpdate(float elapsed,int updateMsg,char* newIP)
{
...
int bb_serverSend(char* dataToSend,int dataSize,int typeID,long destination,int protocol)
{
	if(!Server)
	{
...
char* bb_serverReceive(unsigned long & fromID,int & typeID)
{
...
unsigned long bb_clientConnect(const char* HostIP,unsigned short Port)
{
...
char* bb_clientGetLastError(unsigned long clientID)
{
...
int bb_serverDisconnectClient(unsigned long ID)
{
	if(!Server) return 1;
...
int	bb_peerSend(char *IP,unsigned short port,char *data,int typeID,int dataSize,bool safe)
{
	...
	if(!(IP[0] >= 48 && IP[0] <= 57))
	{
		...
		char newData[sizeof(unsigned long)];
		unsigned long newid = newID;

		memcpy(newData,&newid,sizeof(unsigned long));

        
		if(P2P->SendTo(newID,new cUDPpacket(0,0,(char*)newData,TYPE_DEMAND,sizeof(unsigned long),true)))
		{
		...
		if(P2P->SendTo(newID,new cUDPpacket(0,1,data,typeID,dataSize,safe)))
		{
			//une erreur s'est produite
			return -1;
		}
...
int	bb_peerSend(long peerID,char *data,int typeID,int dataSize,bool safe)
{
	if(!P2P)
	{
		P2P = new cPeer2Peer();
	...
	//on va envoyer le message au Peer
	if(P2P->SendTo(peerID,new cUDPpacket(0,1,data,typeID,dataSize,safe)))
	{
		//une erreur s'est produite
		return 1;
	}
...
char* bb_peerReceive(long *fromPeerID,int *typeID)
{
...
int	bb_peerDelete(unsigned long baboNetID,bool instant)
{
...
int bb_peerGetIPport(unsigned long babonetID,char *IP,unsigned short *Port)
{
	if(!P2P) return 1;
...
unsigned long bb_peerGetBytesSent()
{
	if(P2P)
	{
...
unsigned long bb_peerGetBytesReceived()
{
	if(P2P)
	{
