		...
		for( cClient *c = Clients ; c ; c=c->Next )
		{
			...
			if( tRecv )
			{
				...
				if ((nbytes = recv(c->FileDescriptor, buf, sizeof(buf), 0)) <= 0)
				{
					...
					if (nbytes == 0)
					{
						printf("client ID %li disconnected (peer closed TCP)\n", c->NetID);
					}
					else
					{
						printf("client ID %li disconnected (recv error %i)\n", c->NetID, nbytes);
					}
				...
				else
				{
					...
					if(c->ReceiveStream(nbytes,buf))
					{
						...
						printf("client ID %li disconnected (BaboNet stream rejected; packet ID / framing mismatch)\n", c->NetID);
...
long cServer::UpdateConnections(char *newIP)
{
	...
	for(cIncConnection *c = PendingConnections;c;c=c->Next)
	{
		...
		else if(u > 0)
		{
			...
			printf("New client IP : %s Socket : %i UDPport : %i\n", inet_ntoa(c->IP.sin_addr), c->NewFD, c->UDPport);
...
long cServer::CreateClient(sockaddr_in *ip, int fileDescriptor,unsigned short udpPort, UINT4 pid)
{
	//connection TCP
	nbClient++;
