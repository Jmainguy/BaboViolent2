...
//
// Pour cr�er une billboard static (mais tri� comme les particule) pour faire du gazon � terre surtout
//
									...
									unsigned int dstBlend)
{
	...
	// On va tout suite l'ajouter au bon endroit pour pas avoir des tri important � faire tantot
...
//
// Pour cr�er une particle avec plein d'information de random dessus
//
void			dkpCreateParticleEx(const CVector3f & positionFrom,
									...
									unsigned int dstBlend)
{
	// On d�fini combient on en emet
	int total = rand((int)particleCountFrom, (int)particleCountTo);

	// On trouve le right et le up vector (la direction �tant notre front)
	CVector3f right, up;
...
//
// Pour cr�er une particle ex � partir d'un preset
//
...
//
// Pour setter la direction et la force de la gravit�
//
...
//
// Pour effacer le tout, � la fin du programme
//
void			dkpShutDown()
{
	for (int i=0;i<(int)CDkp::particles.size();i++)
...
int				dkpUpdate(float delay)
{
	...
	// On update notre delait g�n�ral
	CDkp::delay = delay;
	...
	// Maintenant, il faut trier ses particles par rapport � la camera
