...
int	cServer::ReceiveDatagram(cPacket *packet,sockaddr_in fromIP)
{
	...
	else
	{
		...
		UINT4 pid;
		memcpy(&pid,packet->Data,sizeof(UINT4));
...
INT4 cServer::ReceivePacketsFromClients()
{
    ...
    // run through the existing connections looking for data to read
		...
		//			FD_CLR(i, &master); // remove from master set
		//
		//			int disconnectedClient = getClientByFD((UINT4)i)->NetID * -1;
		//
		//			//on va enlever le client de la liste des clients
		//			RemoveClient(getClientByFD((UINT4)i));
...
INT4 cServer::UpdateConnections(char *newIP)
{
	...
	for(cIncConnection *c = PendingConnections;c;c=c->Next)
	{
		...
		{
			INT4 newID = CreateClient(&(c->IP),c->NewFD,c->UDPport, c->Pid);
...
void cServer::AcceptConnection(UINT4 id)
{
	cIncConnection *ic=0;
...
INT4 cServer::CreateClient(sockaddr_in *ip, int fileDescriptor,unsigned short udpPort, UINT4 pid)
{
	//connection TCP
	nbClient++;
...
int cServer::Send(char *dataToSend,int dataSize,int typeID,INT4 destination,int protocol)
{
...
cClient* cServer::getClientByID(UINT4 netid)
{
