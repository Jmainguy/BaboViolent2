...
// Le pointeur sur le writting qui a pr�sentemnt le focus
Writting * writting;
...
{
	// Bon y a des fucks l� avec les caract�res qui ont des accents
	if (caracter > 127)
	{
		for (int i=0;i<1;++i) // Juste pour faire fonctionner le break l�
		{
...
void Writting::updateWritting(float delay)
{
	...
	// Si on d�place avec les fl�ches (must use scancodes: dkiUpdate fills allState from SDL_GetKeyboardState)
